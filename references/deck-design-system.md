...
- **Punctuation obeys the brand's voice_notes.** Some brands ban specific marks entirely (e.g. a hard no-em-dashes rule). Check voice_notes before writing a single slide and honor it in copy, notes, and Now assets alike.
- **Fonts come from brand-tokens roles, never assumptions:** `body` is the basic text font (some brands set a serif here), `wordmark` styles the header chrome, `mono` stays structural. Scattered quote cards render in the body font, so a serif-body brand gets serif quotes automatically.
